  ...
  methods: {
    ...
    createMap() {
      ...
      this.map = new mapboxgl.Map({
        ...
        center: [1.86768, 41.82046], // starting position [lng, lat]
        zoom: 7,
        trackResize: true,
      });
 ...
 */  },
  ...
  watch: {
    recursPerCanviar: function (val) {
      ...
      this.marker = this.allMarkers.find(
        (marker) => marker.id == val.recursos_id
      );

      // Buscamos el recurso a desactivar
      this.recurs = this.recursos.find(
        (recurs) => recurs.id == val.recursos_id
      );
